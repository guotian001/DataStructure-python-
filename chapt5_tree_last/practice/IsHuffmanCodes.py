...
'''
    ...
    检验是否是哈夫曼编码，
        ...
        2.检验是否含有前缀码
            自己在论坛中回答的思路：
            根据编码构建结点（初始结点设为0，表示非编码结点），如果是编码结点则，将其频率放进去，结点结构中加上是否是待编码结点的flag
            有个缺陷：

            # 这样写有个缺陷，只能识别先短编码，在长编码，即先输入短编码，它是后输入的长编码的前缀码，可以检测出来
            # 但是，如果先输入长编码，在输入比其短的前缀码，就检测不出来了

            如何解决这个问题呢？
                在输出编码的最后检测其是否是叶子结点，是否还有子节点
'''
